      ...
      let live =
        ...
        List.fold_left
          (fun live y ->
            if is_reg y
            then S.add y live
            else (try S.add (M.find y regenv) live with Not_found -> live))
          S.empty
          free
      in
      let r =
        (* そうでないレジスタを探す *)
        List.find (fun r -> not (S.mem r live)) (prefer @ all)
      in
    ...
    | Not_found ->
      ...
      let y =
        ...
        List.find
          (fun y ->
            (not (is_reg y))
            && try List.mem (M.find y regenv) all with Not_found -> false)
          (List.rev free)
      in
...
let find x t regenv =
  if is_reg x
  then x
  else (try M.find x regenv with Not_found -> raise (NoReg (x, t)))
;;
...
and g'_and_restore dest cont regenv exp =
  ...
  try g' dest cont regenv exp with
  | NoReg (x, t) ->
...
and g' dest cont regenv = function
  ...
  | St (x, y, z', i) ->
    ( Ans
        (St (find x Type.Int regenv, find y Type.Int regenv, find' z' regenv, i))
    , regenv )
  ...
  | LdDF (x, y', i) ->
    Ans (LdDF (find x Type.Int regenv, find' y' regenv, i)), regenv
  | StDF (x, y, z', i) ->
    ( Ans
        (StDF
           (find x Type.Float regenv, find y Type.Int regenv, find' z' regenv, i))
    , regenv )
  | IfEq (x, y', e1, e2) as exp ->
    g'_if
      ...
      regenv
      exp
      (fun e1' e2' -> IfEq (find x Type.Int regenv, find' y' regenv, e1', e2'))
      e1
      e2
  | IfLE (x, y', e1, e2) as exp ->
    g'_if
      ...
      regenv
      exp
      (fun e1' e2' -> IfLE (find x Type.Int regenv, find' y' regenv, e1', e2'))
      e1
      e2
  | IfGE (x, y', e1, e2) as exp ->
    g'_if
      ...
      regenv
      exp
      (fun e1' e2' -> IfGE (find x Type.Int regenv, find' y' regenv, e1', e2'))
      e1
      e2
  ...
  | CallCls (x, ys, zs) as exp ->
    if List.length ys > Array.length regs - 1
       || List.length zs > Array.length fregs
    then
      failwith
  ...
  | CallDir (Id.L x, ys, zs) as exp ->
    if List.length ys > Array.length regs || List.length zs > Array.length fregs
    then
      failwith
        (Format.sprintf "cannot allocate registers for arugments to %s" x)
    else
...
and g'_if dest cont regenv exp constr e1 e2 =
  ...
  ( List.fold_left
      (fun e x ->
        ...
        then e
        else seq (Save (M.find x regenv, x), e)) (* そうでない変数は分岐直前にセーブ *)
      (Ans (constr e1' e2'))
      (fv cont)
  , regenv' )
...
let h { name = Id.L x; args = ys; fargs = zs; body = e; ret = t; annot } =
