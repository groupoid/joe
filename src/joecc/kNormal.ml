...
let rec g env = function
  ...
  | Syntax.Mul (e1, e2) ->
    insert_let (g env e1) (fun x ->
        insert_let (g env e2) (fun y -> Mul (x, y), Type.Int))
  | Syntax.Div (e1, e2) ->
    insert_let (g env e1) (fun x ->
  ...
  | Syntax.If (Syntax.Not e1, e2, e3) ->
    g env (Syntax.If (e1, e3, e2)) (* notによる分岐を変換 (caml2html: knormal_not) *)
  ...
  | Syntax.Var x ->
    (* 外部配列の参照 (caml2html: knormal_extarray) *)
  ...
  | Syntax.LetRec
      ( { Syntax.name = x, t; Syntax.args = yts; Syntax.body = e1; annot = ant }
      , e2 ) ->
  ...
  | Syntax.Tuple es ->
    let rec bind xs ts = function
      ...
      | e :: es ->
        ...
        insert_let g_e (fun x -> bind (xs @ [ x ]) (ts @ [ t ]) es)
    in
    bind [] [] es
  ...
  | Syntax.Get (e1, e2) ->
    ...
    | (_, Type.Array t) as g_e1 ->
      insert_let g_e1 (fun x -> insert_let (g env e2) (fun y -> Get (x, y), t))
    | _ -> assert false)
  | Syntax.Put (e1, e2, e3) ->
    insert_let (g env e1) (fun x ->
        insert_let (g env e2) (fun y ->
