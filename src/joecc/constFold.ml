...
let memi x env =
  try match M.find x env with Int _ -> true | _ -> false with
  | Not_found -> false
;;

let memf x env =
  try match M.find x env with Float _ -> true | _ -> false with
  | Not_found -> false
;;

let memt x env =
  try match M.find x env with Tuple _ -> true | _ -> false with
  | Not_found -> false
;;
...
let rec g env = function
  ...
  | Let ((x, t), e1, e2) ->
    (* letのケース (caml2html: constfold_let) *)
    let e1' = g env e1 in
    let e2' = g (M.add x e1' env) e2 in
    Let ((x, t), e1', e2')
  | LetRec ({ name = x; args = ys; body = e1; annot }, e2) ->
    LetRec ({ name = x; args = ys; body = g env e1; annot }, g env e2)
