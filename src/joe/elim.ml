...
let rec f = function  (* 不要定義削除ルーチン本体 (caml2html: elim_f) *)
  ...
  | LetRec ({ name = x, t; args = yts; body = e1; annot }, e2) -> (* let recの場合 (caml2html: elim_letrec) *)
    let e2' = f e2 in
    if S.mem x (fv e2')
    then LetRec ({ name = x, t; args = yts; body = f e1; annot }, e2')
    else (Format.eprintf "eliminating function %s@." x; e2')
  | LetTuple (xts, y, e) ->
