...
let rec g env = function (* 定数畳み込みルーチン本体 (caml2html: constfold_g) *)
  ...
  | Let ((x, t), e1, e2) -> let e1' = g env e1 in let e2' = g (M.add x e1' env) e2 in Let ((x, t), e1', e2') (* letのケース (caml2html: constfold_let) *)
  | LetRec ({ name = x; args = ys; body = e1; annot }, e2) -> LetRec ({ name = x; args = ys; body = g env e1; annot }, g env e2)
  | LetTuple (xts, y, e) when memt y env -> List.fold_left2 (fun e' xt z -> Let (xt, Var z, e')) (g env e) xts (findt y env)
